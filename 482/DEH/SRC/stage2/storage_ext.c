	...
	if (p == vsh_process)
	{
                ...
                if (patch != 0) //&& !condition_game_ext_psx)
		{
			switch(vsh_check)
			{
				case VSH_DECR_HASH:
					#ifdef DEBUG
					DPRINTF("Patching Video mode in DECR VSH..\n");
					#endif
					if(decr_vmode_patch_offset) copy_to_user(&patch, (void *)(decr_vmode_patch_offset+0x10000), 4);
					#ifdef DEBUG
					DPRINTF("Offset: 0x%08X | Data: 0x%08X\n", (uint32_t)decr_vmode_patch_offset, (uint32_t)patch);
					#endif
				break;
