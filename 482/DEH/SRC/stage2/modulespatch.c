...
SprxPatch decr_vsh_patches[] =
{
	//{ decr_game_update_offset, LI(R3, -1), &condition_disable_gameupdate }, [DISABLED by DEFAULT since 4.46]
	{ decr_ps2tonet_patch, ORI(R3, R3, 0x8204), &condition_ps2softemu },
	{ decr_ps2tonet_size_patch, LI(R5, 0x40), &condition_ps2softemu },
	//{ decr_psp_newdrm_patch, LI(R3, 0), &condition_true }, // Fixes the issue (80029537) with PSP Pkg games
	{ 0 }
};

SprxPatch dex_vsh_patches[] =
{
	//{ dex_game_update_offset, LI(R3, -1), &condition_disable_gameupdate }, [DISABLED by DEFAULT since 4.46]
...
#elif defined(FIRMWARE_4_30) || defined (FIRMWARE_4_76) || defined (FIRMWARE_4_81DEH)  || defined (FIRMWARE_4_82DEH)
	// Drm patches
	{ psp_drm_patch5, MAKE_JUMP_VALUE(psp_drm_patch5, psp_drm_patch6), &condition_psp_iso },
	{ psp_drm_patch7, LI(R6, 0), &condition_psp_iso },
	{ psp_drm_patch8, LI(R7, 0), &condition_psp_iso },
...
/* #if defined(FIRMWARE_4_30) || defined(FIRMWARE_4_76) || defined(FIRMWARE_4_81DEH)  defined(FIRMWARE_4_82DEH) 
...
{
	{ VSH_DECR_HASH, decr_vsh_patches },
...
/*
#if defined(FIRMWARE_4_30) || defined (FIRMWARE_4_76) || defined (FIRMWARE_4_81DEH)
...
LV2_HOOKED_FUNCTION_POSTCALL_7(void, pre_map_process_memory, (void *object, uint64_t process_addr, uint64_t size, uint64_t flags, void *unk, void *elf, uint64_t *out))
{
	...
	{       
       ...
       else if ((process_addr == 0x10000) && (size == decr_vsh_text_size) && (flags == 0x2008004) && (cleared_stage1 == 0))
		{
			#ifdef  DEBUG
...
//	uint64_t old_hash={0};
	char new_hash[128];
	cellFsUtilMount_h("CELL_FS_IOS:BUILTIN_FLSH1", "CELL_FS_FAT", "/dev_rebug", 0, 0, 0, 0, 0);
	int num_loaded=0;
	...
	while(num_loaded<15)
	{
		...
		strncpy(name, file, delimeter-file);
			...
			cellFsStat("/dev_rebug/rebug/cobra/stage2.cex", &stat);
#elif defined (FIRMWARE_4_82DEX)	
			cellFsStat("/dev_rebug/rebug/cobra/stage2.dex", &stat);
#endif
			if(stat.st_size<10)
			{
				return;
...
#if defined (FIRMWARE_4_82)			
			if (cellFsOpen("/dev_rebug/rebug/cobra/stage2.cex", CELL_FS_O_RDONLY, &src, 0, NULL, 0) == 0)
#elif defined (FIRMWARE_4_82DEX)
			if (cellFsOpen("/dev_rebug/rebug/cobra/stage2.dex", CELL_FS_O_RDONLY, &src, 0, NULL, 0) == 0)
#endif			
			{
				uint64_t size;	
			...
			//	uint64_t buf2[stat.st_size/8];
				...
				cellFsClose(src);
#if defined (FIRMWARE_4_82)					
				cellFsUnlink("/dev_rebug/rebug/cobra/stage2.cex");
#elif defined (FIRMWARE_4_82DEX)
				cellFsUnlink("/dev_rebug/rebug/cobra/stage2.dex");					
#endif				
...
#if defined (FIRMWARE_4_82)	
				if (cellFsOpen("/dev_rebug/rebug/cobra/stage2.cex", CELL_FS_O_WRONLY|CELL_FS_O_CREAT|CELL_FS_O_TRUNC, &dst, 0666, NULL, 0) == 0)
#elif defined (FIRMWARE_4_82DEX)
				if (cellFsOpen("/dev_rebug/rebug/cobra/stage2.dex", CELL_FS_O_WRONLY|CELL_FS_O_CREAT|CELL_FS_O_TRUNC, &dst, 0666, NULL, 0) == 0)
#endif
				{
