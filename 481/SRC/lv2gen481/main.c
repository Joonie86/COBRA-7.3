...
static void patch_elf(char *elf, char *stage0, char *bin, uint64_t address, uint64_t stage0_addr, char *stage0_base)
{
	...
	printf("%s\n", stage0_base);
	FILE *base=fopen(stage0_base, "rb");
	uint8_t buf_base[4096];
	memcpy(buf1+0x10000+0x3d98, buf_base, fread(buf_base, 1, sizeof(buf_base), base));
	fclose(base);
...
{
	char *payload, *stage0, *self_input, *self_output, *stage0_base;
	uint32_t payload_addr;
	uint32_t stage0_addr;
		
	if (argc < 8)
	{
		printf("Usage: %s stage0 stage1 self_input self_output payload_addr stage0_addr stage0_base\n", argv[0]);
		return -1;
	}
	...
	patch_elf("temp.elf", stage0, payload, payload_addr, stage0_addr, stage0_base);	
	command15("scetool", "-v", "--sce-type=SELF", "--compress-data=TRUE", "--compress-data=TRUE", "--skip-sections=FALSE", 
		 "--self-auth-id=1050000003000001", "--self-add-shdrs=TRUE", "--self-vendor-id=05000002", "--self-type=LV2", 
	         "--self-fw-version=0004008100000000", "--key-revision=0", "--self-app-version=0004008100000000", "--encrypt", "temp.elf", self_output);
