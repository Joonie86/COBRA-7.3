...
#if defined (FIRMWARE_4_84DEH) || defined (FIRMWARE_4_84DEX)
	// Drm patches
...
#elif defined (FIRMWARE_4_84DEX) || defined (FIRMWARE_4_84DEH) 
LV2_HOOKED_FUNCTION_COND_POSTCALL_2(int, pre_modules_verification, (uint32_t *ret, uint32_t error))
{
...
LV2_HOOKED_FUNCTION_POSTCALL_7(void, pre_map_process_memory, (void *object, uint64_t process_addr, uint64_t size, uint64_t flags, void *unk, void *elf, uint64_t *out))
{
	...
	{	
		...
		else if ((process_addr == 0x10000) && (size == decr_vsh_text_size) && (flags == 0x2008004) && (cleared_stage0 == 0))
		{
			#ifdef	DEBUG
				DPRINTF("Making DECR VSH text writable, Size: 0x%lx\n", size);   
			#endif
...
{	
	...
	if(param==1) //enable netemu
	{
		...
		{
			case VSH_DECR_HASH:
			copy_to_process(vsh_process, &decr_vsh_patches[0].data, (void *)(uint64_t)(0x10000+decr_vsh_patches[0].offset), 4);
			copy_to_process(vsh_process, &decr_vsh_patches[1].data, (void *)(uint64_t)(0x10000+decr_vsh_patches[1].offset), 4);
			break;
	...
	if(param==0) //restore
	{
		...
		{
			case VSH_DECR_HASH:		
			copy_to_process(vsh_process, &decr_vsh_patches[2].data, (void *)(uint64_t)(0x10000+decr_vsh_patches[2].offset), 4);
			copy_to_process(vsh_process, &decr_vsh_patches[3].data, (void *)(uint64_t)(0x10000+decr_vsh_patches[3].offset), 4);
			break;
