...
static int patch_emu(char *payload_map_file)
{
	...
	if (ret == 0)
	{
		*(uint32_t *)&ps2_netemu[savedata_patch] = swap32(LI(0, 0x26F4)); //previously 0x774 or 0x26F4
	}
...
int main(int argc, char *argv[])
{
	...
	command15("scetool", "-v", "--sce-type=SELF", "--compress-data=TRUE", "--compress-data=TRUE", "--skip-sections=FALSE", 
		 "--self-auth-id=1020000401000001", "--self-add-shdrs=TRUE", "--self-vendor-id=02000003", "--self-type=LV2", 
	         "--self-fw-version=0004003000000000", "--key-revision=0", "--self-app-version=0004007500000000", "--encrypt", "temp.elf", self_output);
